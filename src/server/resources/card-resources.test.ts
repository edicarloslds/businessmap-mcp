...
describe('CardResourceHandler', () => {
  it('bounds and compacts the default board cards resource', async () => {
    const getCardsPage = jest.fn().mockResolvedValue({
      data: [{ card_id: 4, board_id: 12, title: 'Card', description: 'Omitted' }],
      pagination: { all_pages: 2, current_page: 1, results_per_page: 50 },
    });
    ...
    const registration = registerResource.mock.calls.find(([name]) => name === 'cards');
    const callback = registration?.[3] as (
      uri: URL,
      variables: Record<string, string>
    ) => Promise<{ contents: Array<{ text: string }> }>;

    const response = await callback(new URL('businessmap://boards/12/cards'), {
      board_id: '12',
    });
    ...
    expect(getCardsPage).toHaveBeenCalledWith(12, { page: 1, per_page: 50 });
