# Agent setup for `businessmap-mcp`

This is a deepsec scanning workspace. Project `businessmap-mcp` was just registered
(target: `..`). Setup is incomplete — `data/businessmap-mcp/INFO.md`
...
2. **Fill in `data/businessmap-mcp/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id businessmap-mcp
pnpm deepsec process --project-id businessmap-mcp
```
