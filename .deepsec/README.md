...
Currently configured project: `businessmap-mcp` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/businessmap-mcp/SETUP.md` to fill in
   `data/businessmap-mcp/INFO.md`.
...
`--project-id businessmap-mcp` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/businessmap-mcp/`.
